...
service_label="com.nasim.local-ai.ollama"
...
install_plist() {
  ...
  mkdir -p "$user_home/Library/Logs/local-ai"
  cp "$template" "$plist"
  ...
  plutil -replace StandardErrorPath -string "$user_home/Library/Logs/local-ai/ollama-server.error.log" "$plist"
  plutil -replace StandardOutPath -string "$user_home/Library/Logs/local-ai/ollama-server.log" "$plist"
