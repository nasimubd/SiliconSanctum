...
#[test]fn exhausts(){let c=ContextCapacity::new(1).unwrap();let mut s=SessionState::new(c);assert_eq!(generate(&mut B,&mut s,c,VerifyWidth::new(1).unwrap(),2),Err(SpeculativeError::ContextExhausted));}
