...
impl TelemetrySource for FailingSource {
    fn snapshot(&mut self) -> Result<DashboardSnapshot, DashboardError> {
        Err(DashboardError::InvalidMetric)
    }
}
#[test]
fn skips_source_when_paused() {
    let initial = DashboardSnapshot::new(
        TokenRate::new(1.0).unwrap(),
    ...
    let mut state = DashboardState::new(initial.clone());
    state.paused = true;
    state.refresh(&mut FailingSource).unwrap();
