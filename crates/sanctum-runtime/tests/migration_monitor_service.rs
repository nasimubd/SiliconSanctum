...
fn renders_restartable_monitor_arguments() {
    ...
    assert_eq!(dictionary["KeepAlive"].as_dictionary().unwrap()["SuccessfulExit"].as_boolean(), Some(false));
    let arguments = dictionary["ProgramArguments"].as_array().unwrap();
    assert_eq!(arguments[0].as_string(), Some("/Applications/Sanctum & Tools/sanctum-migrate"));
