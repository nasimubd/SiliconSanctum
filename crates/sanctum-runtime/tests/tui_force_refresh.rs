...
#[test]
fn forces_refresh_while_paused() {
    let mk = |rate| {
    ...
    let mut state = DashboardState::new(mk(1.0));
    state.paused = true;
    state.force_refresh(&mut Source(mk(2.0))).unwrap();
    assert!((state.snapshot.token_rate.get() - 2.0).abs() < f64::EPSILON);
}
